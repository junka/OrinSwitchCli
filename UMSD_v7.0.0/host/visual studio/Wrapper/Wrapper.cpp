...
int readC45Register(unsigned int phyAddr, unsigned int devAddr, unsigned int regAddr)
{
    MRegMatlabAccessClass^ regAccObj = nullptr;
    if (NULL != pDeviceHandle)
    {
    ...
    return regAccObj->XMDIOReadRegister(g_USBPort, phyAddr, devAddr, regAddr);
}

int writeC45Register(unsigned int phyAddr, unsigned int devAddr, unsigned int regAddr, unsigned int data)
{
    MRegMatlabAccessClass^ regAccObj = nullptr;
    if (NULL != pDeviceHandle)
    {
    ...
    return regAccObj->XMDIOWriteRegister(g_USBPort, phyAddr, devAddr, regAddr, data);
}
