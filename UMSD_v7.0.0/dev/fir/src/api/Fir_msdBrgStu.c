...
typedef enum
{
    Fir_LOAD_PURGE_STU_ENTRY = 5,
    Fir_GET_NEXT_STU_ENTRY =6 
} FIR_MSD_STU_OPERATION;

static MSD_STATUS Fir_stuOperationPerform
(
    IN      MSD_QD_DEV           *dev,
    IN      FIR_MSD_STU_OPERATION    stuOp,
    INOUT   MSD_U8               *valid,
	INOUT   FIR_MSD_STU_ENTRY        *stuEntry
);

static MSD_STATUS Fir_stuSetSTUData
(
IN    MSD_QD_DEV           *dev,
IN    FIR_MSD_STU_ENTRY        *entry
);

static MSD_STATUS Fir_stuGetSTUData
(
IN    MSD_QD_DEV           *dev,
OUT    FIR_MSD_STU_ENTRY       *entry
);
...
)
{
    ...
    MSD_DBG_INFO(("Fir_gstuGetEntryNext Called.\n"));
	...
	else
	{
		...
		retVal = Fir_stuOperationPerform(dev, Fir_GET_NEXT_STU_ENTRY, &valid, &entry);
		if (retVal != MSD_OK)
		{
			MSD_DBG_ERROR(("Fir_stuOperationPerform Fir_GET_NEXT_STU_ENTRY returned: %s.\n", msdDisplayStatus(retVal)));
		}
		else
		{
			...
			{
				MSD_DBG_INFO(("Fir_gstuGetEntryNext Exit, MSD_NO_SUCH.\n"));
				retVal = MSD_NO_SUCH;
			}
			else
			{
    ...
    MSD_DBG_INFO(("Fir_gstuGetEntryNext Exit.\n"));
	return retVal;
}
...
)
{
    ...
    MSD_DBG_INFO(("Fir_gstuAddEntry Called.\n"));
	...
	else
	{
		...
		for (port = 0; port < dev->maxPorts; port++)
		{
			...
			{
				entry.portState[port] = (FIR_MSD_PORT_STP_STATE)0;
			}
		...
		{
			retVal = Fir_stuOperationPerform(dev, Fir_LOAD_PURGE_STU_ENTRY, &valid, &entry);
			if (retVal != MSD_OK)
			{
				MSD_DBG_ERROR(("Fir_stuOperationPerform add entry returned: %s.\n", msdDisplayStatus(retVal)));
			}
		}
	}

    MSD_DBG_INFO(("Fir_gstuAddEntry Exit.\n"));
	return retVal;
}
...
)
{
    ...
    MSD_DBG_INFO(("Fir_gstuDelEntry Called.\n"));
	...
	else
	{
		...
		if (retVal != MSD_OK)
		{
			MSD_DBG_ERROR(("Fir_stuOperationPerform Delete entry returned: %s.\n", msdDisplayStatus(retVal)));
		}
	}

    MSD_DBG_INFO(("Fir_gstuDelEntry Exit.\n"));
	return retVal;
}
...
)
{
	...
	MSD_DBG_INFO(("Fir_gstuFindSidEntry Called.\n"));
		...
		else
		{
			*found = MSD_FALSE;
			...
			else
			{
				...
				{
					MSD_DBG_INFO(("Fir_gstuFindSidEntry Exit, MSD_NO_SUCH\n"));
					retVal = MSD_OK;
				}
				else
				{
	...
	MSD_DBG_INFO(("Fir_gstuFindSidEntry Exit.\n"));
	return retVal;
}
...
)
{
	...
	msdMemSet(&entry, 0, sizeof(FIR_MSD_STU_ENTRY));
	entry.sid = 0x3f;
	...
	{
		status = Fir_gstuGetEntryNext(dev, &entry);
		...
		MSG(("%-5x%x %x %x %x %x %x %x %x %x %x %x   \n", entry.sid,
			...
			entry.portState[6], entry.portState[7], entry.portState[8], entry.portState[9]));
...
)
{
	...
	data1 |= (MSD_U32)(((MSD_U32)entry->portState[7] & (MSD_U32)3) << 14) | (MSD_U32)(((MSD_U32)entry->portState[6] & (MSD_U32)3) << 12)
	...
	data2 |= (MSD_U16)(((MSD_U16)entry->portState[9] & (MSD_U16)3) << 2)
		| (MSD_U16)((MSD_U16)entry->portState[8] & (MSD_U16)3);


	retVal = msdSetAnyReg(dev->devNum, FIR_GLOBAL1_DEV_ADDR, FIR_VTU_DATA1_REG, data1);
	if(retVal != MSD_OK)
	{
	...
	retVal = msdSetAnyRegField(dev->devNum, FIR_GLOBAL1_DEV_ADDR, FIR_VTU_DATA2_REG, (MSD_U8)0, (MSD_U8)6, data2);
	if(retVal != MSD_OK)
	{
...
)
{
	...
	retVal = msdGetAnyReg(dev->devNum, FIR_GLOBAL1_DEV_ADDR, FIR_VTU_DATA1_REG, &data1);
    if(retVal != MSD_OK)
    {
	...
	retVal = msdGetAnyReg(dev->devNum, FIR_GLOBAL1_DEV_ADDR, FIR_VTU_DATA2_REG, &data2);
    if(retVal != MSD_OK)
    {
	...
	entry->portState[9] = (FIR_MSD_PORT_STP_STATE)(MSD_U16)((MSD_U16)(data2 >> 2) & (MSD_U16)0x3);
	entry->portState[8] = (FIR_MSD_PORT_STP_STATE)(MSD_U16)(data2 & (MSD_U16)0x3);

	entry->portState[7] = (FIR_MSD_PORT_STP_STATE)(MSD_U32)((MSD_U32)(data1 >> 14) & (MSD_U32)0x3);
	entry->portState[6] = (FIR_MSD_PORT_STP_STATE)(MSD_U32)((MSD_U32)(data1 >> 12) & (MSD_U32)0x3);
	entry->portState[5] = (FIR_MSD_PORT_STP_STATE)(MSD_U32)((MSD_U32)(data1 >> 10) & (MSD_U32)0x3);
	entry->portState[4] = (FIR_MSD_PORT_STP_STATE)(MSD_U32)((MSD_U32)(data1 >> 8) & (MSD_U32)0x3);
	entry->portState[3] = (FIR_MSD_PORT_STP_STATE)(MSD_U32)((MSD_U32)(data1 >> 6) & (MSD_U32)0x3);
	entry->portState[2] = (FIR_MSD_PORT_STP_STATE)(MSD_U32)((MSD_U32)(data1 >> 4) & (MSD_U32)0x3);
	entry->portState[1] = (FIR_MSD_PORT_STP_STATE)(MSD_U32)((MSD_U32)(data1 >> 2) & (MSD_U32)0x3);
	entry->portState[0] = (FIR_MSD_PORT_STP_STATE)(MSD_U32)(data1 & (MSD_U32)0x3);

    return MSD_OK;
}

/*******************************************************************************
* Fir_stuOperationPerform
*
* DESCRIPTION:
...
)
{
    ...
    if (stuOp == Fir_LOAD_PURGE_STU_ENTRY)
    {
		if (*valid == (MSD_U8)1)
        {
            /* set the Port State for all the ports */
            retVal = Fir_stuSetSTUData(dev,entry);
            if(retVal != MSD_OK)
            {
                msdSemGive(dev->devNum, dev->vtuRegsSem);
                return retVal;
            ...
            /* Set the valid bit (FIR_QD_REG_VTU_VID_REG) */
        ...
        {
            /* Clear the valid bit (FIR_QD_REG_VTU_VID_REG) */
			data = 0;
			retVal = msdSetAnyReg(dev->devNum, FIR_GLOBAL1_DEV_ADDR, FIR_VTU_VID_REG, data);
            if(retVal != MSD_OK)
            {
    ...
    /* Set the SID register (FIR_QD_REG_STU_SID_REG) */
    ...
    if(stuOp == Fir_GET_NEXT_STU_ENTRY)
    {
        ...
        /****************** Fir_get the valid bit *******************/
		retVal = msdGetAnyRegField(dev->devNum, FIR_GLOBAL1_DEV_ADDR, FIR_VTU_VID_REG, (MSD_U8)12, (MSD_U8)1, &data);
        if(retVal != MSD_OK)
        {
            msdSemGive(dev->devNum, dev->vtuRegsSem);
            return retVal;
        ...
        *valid = (MSD_U8)data;
		...
		if (*valid != (MSD_U8)0)
        {
            /* Fir_get the Port State for all the ports */
            retVal = Fir_stuGetSTUData(dev,entry);
            if(retVal != MSD_OK)
            {
                msdSemGive(dev->devNum, dev->vtuRegsSem);
                return retVal;
